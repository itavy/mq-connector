...
class RabbitMQ {
  ...
  async subscribe({
    ...
    try {
      ...
      return this.subscribeToQueue(Object.assign({}, parseResult, {
        consumer,
        ch,
      }));
  ...
  async checkQueue({
    ...
    try {
      ...
      const { queue: assertedQueue } = await ch.assertQueue(queue, subscribeOptions);
   ...
   * @private
   */
  async checkExchange({ exchange, channel }) {
    if (exchange === '') {
      return true;
    }
    try {
