...
class RabbitMQ {
  ...
  subscribe({
    ...
    return this.getSubscribeChannel()
      .then(ch => this.parseSubscribeOptions({
  ...
  subscribeToQueue({
    ...
    return new Promise((resolve) => {
      ...
      const localConsumer = qMessage => new Promise((resolveConsumer, reject) => {
        consumer({
          nack: () => new Promise((resolveNAck) => {
          ...
          .then(() => {
            ...
            return ch.ack(qMessage);
          })
          .then(() => resolveConsumer())
      ...
      return ch.consume(queue, localConsumer, { noAck: false })
        .then(({ consumerTag }) => resolve({
