...
class RabbitMQ {
  ...
  sendMessageToMQ({
    ...
    return new Promise((resolve, reject) => {
      const publishResult = ch.publish(exchange, queue, message, options, error => {
        if (error) {
          const errMsg = `Confirm error received from server when sending message on exchange "${exchange}" queue "${queue}" ${error}`;
          this.debug(errMsg);
          return reject(Reflect.construct(IError, [{
            name: 'MQ_PUBLISH_MESSAGE_ERROR_NACK',
      ...
      if (!publishResult) {
        ...
        this.debug(errMsg);
        throw Reflect.construct(IError, [{
          name: 'MQ_PUBLISH_MESSAGE_ERROR_BUFFER_FULL',
          source: `${this.sourceIdentifier}.publishMessage`,
          message: errMsg,
          severity: 'WARNING'
        }]);
  ...
  getConnection() {
    ...
    return _asyncToGenerator(function* () {
      if (_this9.connection) {
        return _this9.connection;
      }
      if (_this9.connectionFlags.connecting) {
        yield _this9.promiseOnEvent(_this9.rmqEvents, 'createdConnection');
        return _this9.connection;
      }
      _this9.connectionFlags.connecting = true;
      try {
        _this9.connection = yield _this9.amqplib.connect(_this9.mqURI);
        _this9.connectionFlags.connecting = false;
        _this9.debug(`Successful connected to ${_this9.mqURI}`);
        _this9.rmqEvents.emit('createdConnection');
        return _this9.connection;
      } catch (connectError) {
        ...
        throw Reflect.construct(IError, [{
          ...
          source: `${_this9.sourceIdentifier}.connect`,
          message: errMsg,
          severity: 'FATAL',
          cause: connectError
        }]);
  ...
  createChannel({ name, flag, event }) {
    ...
    return _asyncToGenerator(function* () {
      _this10.connectionFlags[flag] = true;
      try {
        const connection = yield _this10.getConnection();
        _this10[name] = yield connection.createConfirmChannel();
        _this10.debug(`Successful created ${name} channel`);
        _this10.rmqEvents.emit(event);
        _this10.connectionFlags[flag] = false;
        return _this10[name];
      } catch (chError) {
        ...
        throw Reflect.construct(IError, [{
          ...
          source: `${_this10.sourceIdentifier}.createChannel`,
          message: errMsg,
          severity: 'FATAL',
          cause: chError
        }]);
  ...
  getPublishChannel() {
    ...
    return _asyncToGenerator(function* () {
      if (_this11.connectionFlags.publish) {
        if (_this11.publishChannel) {
          return _this11.publishChannel;
        }
        if (_this11.connectionFlags.creatingPC) {
          yield _this11.promiseOnEvent(_this11.rmqEvents, 'createdPublishChannel');
          return _this11.publishChannel;
        }
        yield _this11.createChannel({
          name: 'publishChannel',
          flag: 'creatingPC',
      ...
      throw Reflect.construct(IError, [{
        ...
        source: `${_this11.sourceIdentifier}.getPublishChannel`,
        message: errMsg,
        severity: 'FATAL'
      }]);
    })();
  ...
  getSubscribeChannel() {
    ...
    return _asyncToGenerator(function* () {
      if (_this12.connectionFlags.subscribe) {
        if (_this12.subscribeChannel) {
          return _this12.subscribeChannel;
        }
        if (_this12.connectionFlags.creatingSC) {
          yield _this12.promiseOnEvent(_this12.rmqEvents, 'createdSubscribeChannel');
          return _this12.subscribeChannel;
        }
      ...
      throw Reflect.construct(IError, [{
        ...
        source: `${_this12.sourceIdentifier}.getSubscribeChannel`,
        message: errMsg,
        severity: 'FATAL'
      }]);
    })();
  ...
  closeChannel({ name, flag, event }) {
    ...
    return _asyncToGenerator(function* () {
      if (_this13[name] === null) {
        return Promise.resolve();
      }
      if (!_this13.connectionFlags[flag]) {
        _this13.connectionFlags[flag] = true;
        yield _this13[name].waitForConfirms();
        yield _this13[name].close();
        _this13[name] = null;
        _this13.connectionFlags[flag] = false;
        _this13.rmqEvents.emit(event);
        return true;
      }
      return _this13.promiseOnEvent(_this13.rmqEvents, event);
    })();
  }
}
